...
import { CELL_TYPES } from "./model.mjs";
...
function normalizeNamedRange(raw) {
  const source = isRecord(raw) ? raw : {};
  return {
...
function normalizeNamedExpressions(raw) {
  const source = isRecord(raw) ? raw : {};
  return {
...
export function normalizeSpreadsheet(raw) {
  const source = isRecord(raw) ? raw : {};
  return {
